...
let baseTransactionConnections = function (queryBuilder) {
  queryBuilder
    .join("transactions", {
      "users_transactions.txn_id": "transactions.txn_id",
    })
...
let mustIncludeUserById = function (queryBuilder, userId) {
  queryBuilder.where(function () {
    this.where("users_transactions.payer_id", userId).orWhere(
      "users_transactions.payee_id",
      userId
    );
  });
};
...
let mustIncludeUserByUsername = function (queryBuilder, username) {
  if (username) {
    queryBuilder.where(function () {
      this.where("payer.username", username).orWhere(
        "payee.username",
        username
      );
    });
...
module.exports = {
  ...
  profileFeedRelational: function (
    limit,
    beforeId,
    sinceId,
    profileUsername,
