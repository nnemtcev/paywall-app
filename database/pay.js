...
const getPayerBalance = function (
  paymentTransaction,
  localPaymentInfo,
  paymentDataFromServer
) {
...
const getPayeeInfo = function (
  ...
  return pg
    ...
    .then((rows) => {
      ...
      if (
        rows.length === 0 ||
        rows[0].id === parseInt(paymentDataFromServer.payerId)
      ) {
        throw new Error(
          "Invalid payee username:",
          paymentDataFromServer.payeeUsername
        );
      }
...
const updatePayerBalance = function (
  paymentTransaction,
  paymentDataFromServer,
  localPaymentInfo
) {
...
const updatePayeeBalance = function (
  paymentTransaction,
  paymentDataFromServer,
  localPaymentInfo
) {
